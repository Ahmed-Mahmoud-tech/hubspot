...
export class HubSpotService {
  ...
  async finishProcess(
    ...
    this.updateProgress(userId, apiKey, {
  ...
  async removeContact(
    ...
    if (updatedGroup.length < 2) {
      ...
      await this.matchingRepository.delete({ id: groupId });
      this.logger.log(
        `Deleted group ${groupId} - insufficient contacts remaining`,
      );
      return {
        message:
          'Contact removed and marked for deletion. Group deleted because less than 2 contacts remain.',
      };
    } else {
  ...
  private async mergeAllRemainingDuplicates(
    ...
    try {
      ...
      const duplicateGroups = await this.matchingRepository.find({
        where: { userId, apiKey },
      ...
      for (let i = 0; i < pendingMerges.length; i++) {
        ...
        try {
          ...
          const primaryContact = await this.contactRepository.findOne({
            where: {
              hubspotId: merge.primaryAccountId,
              user: { id: userId },
              apiKey,
            },
          });
          ...
          const secondaryContact = await this.contactRepository.findOne({
            where: {
              hubspotId: merge.secondaryAccountId,
              user: { id: userId },
              apiKey,
            },
          });
