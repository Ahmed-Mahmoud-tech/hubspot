...
export class HubSpotService {
  ...
  private async fetchAllContacts(
    ...
    try {
      do {
        ...
        // const first15Results = JSON.stringify(results);
        // console.log(first15Results, 'First 15 contactssss', paging);
  ...
  private async updateContactsInHubSpot(
    ...
    for (const modified of modifiedContacts) {
      ...
      if (contact && contact.hubspotId) {
        try {
          ...
          Object.keys(filteredData).forEach((key) => {
            if (key !== 'email') {
              properties[key] = filteredData[key];
            }
          });
      ...
      // console.log(modified.updatedData, '00000000000000000000000');
    }
  }
  ...
  private async updateHubSpotContact(
    ...
    try {
      ...
      if (response.status === 200) {
        this.logger.log(
          `Successfully updated HubSpot contact ${hubspotId} with properties: ${Object.keys(properties).join(', ')}`,
        );
        return { success: true };
      } else {
        return {
  ...
  private async deleteHubSpotContact(
    ...
    try {
      ...
      if (response.status === 204) {
        this.logger.log(`Successfully deleted HubSpot contact ${hubspotId}`);
        return { success: true };
      } else {
        return {
          success: false,
